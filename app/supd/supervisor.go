...
import (
	...
	"clc.hmu/app/supd/events"
	"clc.hmu/app/supd/faults"
	"clc.hmu/app/supd/logger"
	"clc.hmu/app/supd/process"
	"clc.hmu/app/supd/signals"
	"clc.hmu/app/supd/types"
	"clc.hmu/app/supd/util"
...
func (s *Supervisor) setSupervisordInfo() {
	...
	if ok {
		...
		if err == nil {
			...
			if err != nil {
				log.Warning("supervisord new file [%s] failed: %s", logFile, err)
				// panic(err) // 守护程序的日志不能创建，不再继续执行，以便错误可以得到处理
				s.logger, err = logger.NewLogger("supervisord", "/tmp/supervisord.log", &sync.Mutex{}, logfile_maxbytes, logfile_backups, logEventEmitter)
			}
